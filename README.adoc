...
* Python 3.8+
* https://hatch.pypa.io/latest/[Hatch] (recommended for project management)
...
* **Test Suite Isolation**: How to resolve event loop conflicts between `pytest-asyncio` and `pytest-playwright` (link:tests/patterns/testing/TEST_SUITE_ISOLATION.adoc[See Guide]).
...
[NOTE]
====
We have transitioned our in-depth guides to AsciiDoc to allow for live code snippet inclusion.
====

* **Package Loading**: Strategies for loading packages via `micropip` (link:tests/patterns/loading/LOADING_PATTERNS.adoc[See Guide]).
...
Contributions are welcome! If you have a new pattern or recipe, please see our link:CONTRIBUTING.adoc[contributing guide] for more details.
