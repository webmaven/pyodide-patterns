...
def pytest_configure(config):
    """Validate that the local Python version matches the required version for the selected Pyodide version."""
    ...
    if not current_python_version.startswith(required_python_version.rsplit('.', 1)[0]):
        print(f"WARNING: Pyodide version {pyodide_version} requires Python {required_python_version}, "
              f"but the local version is {current_python_version}. Proceeding anyway.")
        # pytest.exit(
        #     f"Pyodide version {pyodide_version} requires Python {required_python_version}, "
