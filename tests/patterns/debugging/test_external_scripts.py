...
def test_external_script_success(page: Page, live_server: str):
    """
    Load a page that includes a valid external script and verify it runs
    without error.
    """
...
def test_external_script_404_failure(page: Page, live_server: str):
    """
    Navigate to a page that references a missing external script and assert
    a requestfailed event.
    """
    messages = []
    ...
    assert any("script_not_found.js" in r.url for r in messages), (
        "Missing script request not captured"
    )
...
def test_external_script_runtime_error(page: Page, live_server: str):
    """
    Load a page with an external script that throws an error and capture
    the pageerror event.
    """
    errors = []
