...
def test_load_package_happy_path(page: Page, live_server: str):
    ...
    result = page.evaluate("window.pyodide.runPython('import numpy; numpy.array([1, 2, 3]).sum().item()')")
    assert result == 6
...
def test_load_package_not_found(page: Page, live_server: str):
    ...
    success = page.evaluate("async () => await loadPkg('a-package-that-does-not-exist')")
    assert success is False

    expect(page.locator("#status")).to_contain_text("No known package with name 'a-package-that-does-not-exist'")
...
def test_load_from_imports_happy_path(page: Page, live_server: str):
    ...
    result = page.evaluate("window.pyodide.runPython('numpy.array([1, 2, 3]).sum().item()')")
    assert result == 6
...
def test_micropip_install_from_pypi(page: Page, live_server: str):
    ...
    expect(page.locator("#status")).to_have_text("Pyodide and micropip loaded. Ready.", timeout=60000)

    success = page.evaluate("async () => await installPkg('tinycss2')")
    assert success is True

    # Verify the package is usable
    result = page.evaluate("window.pyodide.runPython('import tinycss2; tinycss2.__version__')")
...
def test_micropip_install_from_url_with_deps(page: Page, live_server: str):
    ...
    wheel_url = f"{live_server}/_my_local_package/dist/my_local_package-0.1.0-py3-none-any.whl"
    success = page.evaluate(f"async () => await installPkg('{wheel_url}')")
    assert success is True

    # Verify both the package and its dependency are usable
    result_pkg = page.evaluate("window.pyodide.runPython('import my_local_package; my_local_package.a_test_function()')")
    assert result_pkg == "This is from my_local_package"
    result_dep = page.evaluate("window.pyodide.runPython('import tinycss2; tinycss2.__version__')")
...
def test_micropip_install_cors_failure_new(page: Page, live_server: str, cross_origin_server: str):
    """Tests that micropip.install() fails with a CORS error when the server does not send CORS headers."""
    page.goto(f"{live_server}/examples/loading/micropip_install.html")
    expect(page.locator("#status")).to_have_text("Pyodide and micropip loaded. Ready.", timeout=60000)
