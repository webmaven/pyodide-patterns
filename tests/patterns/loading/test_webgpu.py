...
@pytest.mark.skip(reason="WebGPU is often unavailable in headless CI environments without specific hardware and flags.")
def test_webgpu_compute_execution(page: Page, live_server: str):
    ...
    if compute_btn.is_enabled():
        compute_btn.click()
        expect(page.locator("#output")).to_contain_text("Result: [2,4,6,8]", timeout=30000)
    else:
