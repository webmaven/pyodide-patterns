...
@pytest.mark.xfail(reason="Isolated environments often block CDN resources that lack CORP headers, preventing Pyodide from loading.")
def test_shared_memory_gpu_execution(page: Page, isolated_server: str):
    ...
    # 1. Verify Isolation
    expect(page.locator("#isolation-status")).to_have_text("Environment is cross-origin isolated. SAB available.", timeout=30000)
    ...
    expect(page.locator("#output")).to_contain_text("Verification: JS view matches Python result.")
