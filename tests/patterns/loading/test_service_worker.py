...
@pytest.mark.xfail(
    reason=(
        "Service Workers are notoriously unreliable in headless testing environments."
    )
)
def test_service_worker_registration(page: Page, live_server: str):
    ...
    expect(sw_status).to_have_text(
        "Service Worker: Active and Controlling Page", timeout=30000
    )

    # Verify Pyodide loads (likely from cache if run repeatedly, but first
    # time is from network)
...
def test_service_worker_interception(page: Page, live_server: str):
    ...
    # Wait for completion
    expect(page.locator("#pyodide-status")).to_contain_text(
        "Pyodide: Loaded", timeout=60000
    )

    # Check if any requests were served by the service worker
    # Playwright's request object has a 'service_worker' attribute or
    # 'from_service_worker'
