...
def test_worker_pool_parallelism(page: Page, live_server: str):
    ...
    expect(page.locator("#status")).to_have_text("Pool Ready. Running 2 workers.", timeout=60000)
    ...
    assert total_duration < 7.0, f"Batch took too long ({total_duration:.2f}s), parallelism might be broken."
